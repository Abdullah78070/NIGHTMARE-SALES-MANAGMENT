...
interface Props {
    ...
    onOpenInvoice?: (invoice: PurchaseInvoice) => void;
    onDelete?: (id: string) => void;
    initialSelectedId?: string;
}

export const PurchaseList: React.FC<Props> = ({ invoices, onOpenInvoice, onDelete, initialSelectedId }) => {
  ...
  const [filterVendor, setFilterVendor] = useState('الكل');
  const [filterStatus, setFilterStatus] = useState('الكل');

  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const rowRefs = useRef<(HTMLTableRowElement | null)[]>([]);
  
  const vendors = ['الكل', 'توريدات النيل', 'برمجيات المستقبل', 'مكتبة الخليج', 'تقنية العرب'];
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter(inv => {
        ...
        // 3. Vendor
        if (filterVendor !== 'الكل' && inv.vendor !== filterVendor) return false;
  ...
  }, [invoices, idFrom, idTo, dateFrom, dateTo, filterVendor, filterStatus]);
  ...
  const clearFilters = () => {
    ...
    setFilterVendor('الكل');
    setFilterStatus('الكل');
  };

  const getStatusColor = (status: string) => {
      switch(status) {
          case 'محول': return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-[0_0_8px_rgba(16,185,129,0.1)]';
      ...
      <div className="w-full h-full flex flex-col bg-[#111827] shadow-2xl relative rounded-xl overflow-hidden border border-white/5">
        ...
        <div className="sticky top-0 z-30 bg-[#161e2d] border-b border-slate-800 px-4 py-2 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white leading-none">إدارة المشتريات</h1>
              <p className="text-[10px] text-slate-500 mt-1">اضغط مرتين لفتح الفاتورة - Delete للحذف</p>
            </div>
          </div>

          <button 
            onClick={() => setShowFilters(!showFilters)}
            className={`p-1.5 rounded-md transition-all ${showFilters ? 'bg-slate-700 text-slate-300' : 'bg-blue-600 text-white'}`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1 uppercase">
                <Hash size={12} className="text-blue-500" /> نطاق الفاتورة
              </label>
              <div className="flex items-center gap-1">
                <input 
                    ...
                    placeholder="من" 
                    className="w-full bg-[#0b0f1a] border border-slate-700 rounded px-2 py-1 text-xs outline-none focus:border-blue-500 transition-colors text-white" 
                />
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1 uppercase">
                <Calendar size={12} className="text-blue-500" /> الفترة الزمنية
              </label>
              <div className="flex items-center gap-1">
                <input 
                    type="text" 
                    ...
                    className="w-full bg-[#0b0f1a] border border-slate-700 rounded px-1 py-1 text-[10px] text-white outline-none focus:border-blue-500 transition-colors font-mono text-center" 
                />
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1 uppercase">
                <User size={12} className="text-blue-500" /> جهة التوريد
              </label>
              <select 
                value={filterVendor}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-500 flex items-center gap-1 uppercase">
                <Settings2 size={12} className="text-blue-500" /> الحالة
              </label>
          ...
          <table className="w-full text-right border-collapse min-w-[700px]">
            ...
            <thead className="sticky top-0 z-10 bg-[#161e2d] border-b border-slate-700">
              <tr className="text-slate-500 text-[10px] uppercase">
                ...
                <th className="px-6 py-3 font-bold">تاريخ الطلب</th>
                <th className="px-6 py-3 font-bold">اسم المورد</th>
                <th className="px-6 py-3 font-bold text-center">القيمة الإجمالية</th>
                <th className="px-6 py-3 font-bold text-center">حالة القيد</th>
            ...
            <tbody className="divide-y divide-slate-800/30">
              {filteredInvoices.map((inv, index) => (
                <tr 
                  ...
                  className={`group transition-all duration-150 cursor-pointer relative ${
                    selectedIndex === index ? 'bg-blue-600/15' : 'hover:bg-slate-800/40'
                  }`}
                >
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      {selectedIndex === index && <div className="absolute right-0 top-1 bottom-1 w-1 bg-blue-500 rounded-l-full shadow-[0_0_10px_rgba(59,130,246,0.5)]" />}
                      <span className={`font-mono text-xs ${selectedIndex === index ? 'text-blue-400 font-bold' : 'text-slate-500'}`}>
                        #{inv.id}
                      </span>
                    </div>
                  </td>
                  <td className={`px-6 py-3 font-mono text-[11px] transition-colors ${selectedIndex === index ? 'text-blue-300' : 'text-slate-500'}`}>
                    {inv.date}
                  </td>
                  ...
                  <td className="px-6 py-3 text-left flex gap-1 justify-end">
                    <button 
                        ...
                        className={`p-1.5 hover:bg-slate-700 rounded transition-colors ${selectedIndex === index ? 'text-blue-400' : 'text-slate-600'}`}
                        title="فتح الفاتورة"
                    >
              ...
              {filteredInvoices.length === 0 && (
                  <tr>
                      <td colSpan={6} className="text-center py-10 text-gray-500">لا توجد فواتير تطابق الفلترة</td>
                  </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="sticky bottom-0 z-30 bg-[#161e2d] border-t border-slate-800 px-4 py-2 flex justify-between items-center text-[10px] shrink-0">
          <div className="flex gap-4 items-center">
            <div className="flex items-center gap-2 px-2 py-0.5 bg-slate-800/50 rounded border border-slate-700">
              ...
              <span className="text-slate-400 font-medium">مستخدم: مدير النظام</span>
            </div>
          ...
          <div className="flex items-center gap-3">
             <div className="bg-blue-600/20 text-blue-400 px-3 py-0.5 rounded-full border border-blue-500/20 font-bold">
