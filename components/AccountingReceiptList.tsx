...
export const AccountingReceiptList: React.FC<Props> = ({ receipts, onDelete, onEdit, onNavigateAdd }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [dateFilter, setDateFilter] = useState('');
  const [sortConfig, setSortConfig] = useState<{ key: keyof CashReceipt; direction: 'asc' | 'desc' } | null>({ key: 'timestamp', direction: 'desc' });
  ...
  const filteredData = useMemo(() => {
    ...
    if (dateFilter) {
        // Assuming date is DD/MM/YYYY stored as string, we need to match carefully or convert
    ...
    return data;
  }, [receipts, searchTerm, dateFilter, sortConfig]);

  // --- Stats ---
  const stats = useMemo(() => {
    const today = new Date().toLocaleDateString('en-GB');
    return {
        totalToday: receipts.filter(r => r.date === today).reduce((sum, r) => sum + r.amount, 0),
        count: receipts.length,
        maxReceipt: Math.max(...receipts.map(r => r.amount), 0)
    };
  }, [receipts]);

  // --- Handlers ---
  const handleSort = (key: keyof CashReceipt) => {
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 no-print bg-[#1e293b] p-4 rounded-2xl border border-white/5 shadow-lg">
                <div className="flex items-center gap-3">
                    <div className="bg-emerald-600/20 p-3 rounded-xl border border-emerald-500/20 text-emerald-500">
                        <Wallet size={24} />
                    </div>
                    <div>
                        <h1 className="text-xl font-black text-white">سندات استلام النقدية</h1>
                        <p className="text-[11px] text-slate-400 font-bold">سجل كامل لجميع المقبوضات المالية</p>
                    </div>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                    <button onClick={onNavigateAdd} className="flex-1 md:flex-none bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm font-bold transition shadow-lg shadow-emerald-900/20 active:scale-95">
                        <Plus size={16} />
                        سند جديد
                    </button>
            ...
            <div className="hidden print-header">
                <h1 className="text-2xl font-bold text-center">تقرير سندات القبض</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 no-print">
                <div className="bg-[#1e293b] p-4 rounded-xl border border-white/5 shadow-md flex items-center justify-between">
                    <div>
                        <p className="text-slate-400 text-[10px] font-bold uppercase mb-1">إجمالي المحصل اليوم</p>
                ...
                <div className="bg-[#1e293b] p-4 rounded-xl border border-white/5 shadow-md border-r-4 border-r-emerald-600 flex items-center justify-between">
                    <div>
                ...
                <div className="bg-[#1e293b] p-4 rounded-xl border border-white/5 shadow-md flex items-center justify-between">
                    <div>
                        <p className="text-slate-400 text-[10px] font-bold uppercase mb-1">أكبر عملية استلام</p>
                        <div className="flex items-baseline gap-1">
                            <h2 className="text-2xl font-black text-blue-400">{stats.maxReceipt.toLocaleString()}</h2>
                            <span className="text-[10px] text-slate-500 font-bold">EGP</span>
                        </div>
                    </div>
            ...
            <div className="bg-[#1e293b] p-3 rounded-xl border border-white/5 shadow-md mb-4 no-print flex flex-wrap gap-3 items-center sticky top-0 z-10 backdrop-blur-md bg-opacity-90">
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input 
                        ...
                        placeholder="بحث سريع (رقم السند، العميل، المبلغ)..." 
                        className="w-full pr-10 pl-3 py-2 bg-[#0f172a] border border-slate-700 rounded-lg text-xs text-white focus:outline-none focus:border-emerald-500 transition font-bold"
                    />
                </div>
                    ...
                    <table className="w-full text-right border-collapse print-table">
                        <thead>
                            <tr className="bg-[#161e2e] text-slate-400 text-[10px] font-bold uppercase tracking-wider border-b border-white/5">
                                ...
                                <th className="px-6 py-4 cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('payer')}>
                                    <div className="flex items-center gap-1">اسم العميل <ArrowUpDown size={10} className="opacity-0 group-hover:opacity-100" /></div>
                                </th>
                        ...
                        <tbody className="divide-y divide-white/5 text-xs text-slate-300">
                            {filteredData.map((item) => (
                                <tr 
                                    ...
                                    className={`transition-all duration-150 cursor-pointer group ${selectedId === item.id ? 'bg-emerald-600/10 border-l-4 border-l-emerald-500' : 'hover:bg-[#0f172a] border-l-4 border-l-transparent'}`}
                                >
                                    ...
                                    <td className="px-6 py-3 font-bold text-white">{item.payer}</td>
                                    <td className="px-6 py-3 text-slate-400 truncate max-w-[200px]">{item.description}</td>
                                    <td className="px-6 py-3 text-center">
                                        <span className="font-black text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/20 font-mono">
                                    ...
                                    <td className="px-6 py-3 no-print">
                                        <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); if(onEdit) onEdit(item); }} className="text-emerald-400 hover:bg-emerald-500/20 p-2 rounded-lg transition-colors" title="تعديل">
                                                <Edit size={14} />
                                            </button>
